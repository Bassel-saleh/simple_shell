 ...
 * Return: bytes read
 */
ssize_t in_buf(info_t *info, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			rmv_cmnts(*buf);
			build_your_history_list(info, *buf, info->histcount++);
...
/**
 * ginput - gets a line minus the newline
 * @info: parameter struct
 *
 * Return: bytes read
 */
 ...
 * Return: r
 */
ssize_t rbuf(info_t *info, char *buf, size_t *i)
{
	ssize_t r = 0;
 ...
 * Return: s
 */
int g_line(info_t *info, char **ptr, size_t *length)
{
